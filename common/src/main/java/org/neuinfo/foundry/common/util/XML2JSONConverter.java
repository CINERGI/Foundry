...
public class XML2JSONConverter {
    ...
    protected void toXML(Element parentEl, JSONObject parent, Map<String, Namespace> namespaceMap) throws JSONException {
        ...
        if (!attributeNames.isEmpty()) {
            Namespace parentElNamespace = parentEl.getNamespace();
            for (String name : attributeNames) {
                String value = parent.getString(name);
                parentEl.setAttribute(prepAttribute2(name, value, parentElNamespace));
            }
        }
    ...
    public static Attribute prepAttribute2(String qName, String value, Namespace parentElNamespace) {
        // remove the prefix @
        ...
        String localName = toks[1];
        String nsPrefix = toks[0];
        // don't allow attribute redundant namespace prefix (same as element's namespace
        if (parentElNamespace != null && parentElNamespace.getPrefix().equals(nsPrefix)) {
    ...
    public static void cli(String[] args) throws Exception {
        ...
        Option xfOption = OptionBuilder.withArgName("xml-file").hasArg()
                .withDescription("XML file to convert to JSON").create('f');
        xfOption.setRequired(true);
