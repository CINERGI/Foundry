...
public class ElasticSearchIndexerJMSConsumer extends JMSConsumerSupport implements MessageListener {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Option ipOption = OptionBuilder.withArgName("index-path").hasArg()
                .withDescription("Elasticsearch index-path for REST api").create('p');
        Option urlOption = OptionBuilder.withArgName("server-url").hasArg()
                .withDescription("Elasticsearch server url such as 'http://localhost:9200'").create("u");
        Option cmdOption = OptionBuilder.withArgName("command").hasArg()
                .withDescription("command (one of [i,d] where d is for delete index)").create('c');
